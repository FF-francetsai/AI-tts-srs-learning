...
- AI SRS 智慧學習平台 - 核心邏輯（已修正版）
- 修正清單：
- FIX 1 - body overflow 改由 CSS media query 處理
- FIX 2 - Sidebar backdrop 遮罩（點擊外部關閉）
- FIX 3 - header-controls 漢堡選單展開/收合
- FIX 4 - quizQuestion 顏色跟隨主題
- FIX 6 - SRS 按鈕綁定改為事件委派，支援動態生成元素
- FIX 7 - 移除重複的 updateProgressBar，統一用 updateProgress
- FIX 8 - 移除重複的 alert
- FIX 9 - Mermaid SVG 寬度限制
  */
...
renderCurrent: (isModal = false) => {
    ...
    const actualQEl = document.getElementById(isModal ? 'modalQuizQuestion' : 'quizQuestion');
...
checkAnswer: (selected, correct, explain, isModal = false) => {
    ...
    if (explainEl) explainEl.innerHTML = `<strong>解析：</strong><br>${explain}`;
...
init: () => {
    ...
    // 確保全域資料存在（fallback）
    if (!window.lessonData) window.lessonData = (typeof lessonData !== 'undefined' ? lessonData : []);
    if (!window.quizBank) window.quizBank = (typeof quizBank !== 'undefined' ? quizBank : {});
...
renderDetail: (topic) => {
    ...
    modal.innerHTML = `
        ...
        <div class="detail-content glass-panel">
            ...
            <div class="topic-header">
                <h1 style="font-size:1.4rem; font-weight:800; color:var(--accent-color); margin-bottom:8px;">#${topic.id} ${topic.title}</h1>
                        ...
                        <div class="card-back-scroll">
                            ...
                            <div class="srs-actions" style="margin-top: 20px;">
                                <div class="srs-btns-grid">
                                    <button class="srs-btn again" data-grade="0" data-topic-id="${topic.id}" data-srs-source="card">忘了 (Again)</button>
                                    <button class="srs-btn hard" data-grade="1" data-topic-id="${topic.id}" data-srs-source="card">困難 (Hard)</button>
                                    <button class="srs-btn good" data-grade="2" data-topic-id="${topic.id}" data-srs-source="card">答對 (Good)</button>
                                    <button class="srs-btn easy" data-grade="3" data-topic-id="${topic.id}" data-srs-source="card">秒答 (Easy)</button>
                                </div>
                            </div>
            ...
            <div id="quizContainerInModal" class="hidden glass-panel" style="margin-top: 20px; padding: 25px; border: 1px solid var(--accent-color);">
                <div id="modalQuizQuestion" class="quiz-q" style="font-size: 1.1rem; font-weight: 700; margin-bottom: 20px; color: var(--text-main);"></div>
                ...
                <div id="modalQuizFeedback" class="quiz-feedback hidden" style="margin-top: 25px;">
                    ...
                    <div id="modalTopicDetailExplain" class="quiz-explain hidden" style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                    ...
                    <div id="modalSrsActions" class="srs-btns-grid hidden">
                        <button class="srs-btn again" data-grade="0" data-topic-id="${topic.id}" data-srs-source="quiz">忘了 (Again)</button>
                        <button class="srs-btn hard" data-grade="1" data-topic-id="${topic.id}" data-srs-source="quiz">困難 (Hard)</button>
                        <button class="srs-btn good" data-grade="2" data-topic-id="${topic.id}" data-srs-source="quiz">答對 (Good)</button>
                        <button class="srs-btn easy" data-grade="3" data-topic-id="${topic.id}" data-srs-source="quiz">秒答 (Easy)</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    // FIX 6: 使用事件委派綁定 Modal 內 SRS 按鈕
    modal.addEventListener('click', (e) => {
        const btn = e.target.closest('.srs-btn[data-topic-id]');
        if (!btn) return;
        e.stopPropagation();
        ...
        if (App.isSRSMode) {
            ...
            if (nextTid) {
                const allTopics = (window.lessonData || []).flatMap(u => u.sub_units.flatMap(s => s.topics));
                const nextTopic = allTopics.find(t => t.id === nextTid);
                if (nextTopic) { App.loadTopic(nextTopic); return; }
            }
...
_openSidebar: () => {
    ...
    if (!backdrop) {
        ...
        backdrop.onclick = App._closeSidebar;
...
bindEvents: () => {
    // FIX 2 & FIX 3: 漢堡選單（展開 sidebar + header-controls）
    ...
    if (menuBtn) {
        menuBtn.onclick = (e) => {
            ...
            if (window.innerWidth <= 768) {
                ...
                // 桌面：切換 header controls（若有需要）
                if (headerControls) headerControls.classList.toggle('active');
            }
        };
    }
    ...
    if (fontUpBtn) fontUpBtn.onclick = () => {
        App.fontSize += 2;
        if (contentArea) contentArea.style.fontSize = App.fontSize + 'px';
    };
    if (fontDownBtn) fontDownBtn.onclick = () => {
    ...
    if (searchInput) {
        searchInput.oninput = (e) => {
            ...
            if (!val) { App.renderSidebar(window.lessonData); return; }

            const filtered = (window.lessonData || []).map(unit => {
    ...
    // FIX 6: 靜態 SRS 按鈕（HTML 裡的 quizContainer）改用事件委派
    const staticQuizContainer = document.getElementById('quizContainer');
    if (staticQuizContainer) {
        staticQuizContainer.addEventListener('click', (e) => {
            const btn = e.target.closest('.srs-btn');
            if (!btn) return;
            const grade = parseInt(btn.dataset.grade);
            if (isNaN(grade)) return;

            const tid = App.currentTopic ? App.currentTopic.id : null;
            if (tid) {
                SRSManager.updateCard(tid, grade);
                App.updateProgress();
            }

            staticQuizContainer.classList.add('hidden');

            if (App.isSRSMode && tid) {
                const dueIds = SRSManager.getDueTopics();
                const nextTid = dueIds.find(id => id !== tid);
                if (nextTid) {
                    const allTopics = (window.lessonData || []).flatMap(u => u.sub_units.flatMap(s => s.topics));
    ...
    // SRS 複習切換
    const srsToggleBtn = document.getElementById('srsToggle');
    if (srsToggleBtn) {
        srsToggleBtn.onclick = () => {
            App.isSRSMode = !App.isSRSMode;
            srsToggleBtn.classList.toggle('active', App.isSRSMode);
            const dueIds = SRSManager.getDueTopics();
...
// 抽取錯題測驗 UI 渲染（避免重複程式碼）
_renderWrongQuizUI: (title = '⚠️ 錯題補救挑戰', desc = '針對不熟悉的主題進行強化訓練。') => {
    ...
    `;
    // FIX 6: 重新綁定動態生成的 quizContainer
    const dynamicContainer = document.getElementById('quizContainer');
    if (dynamicContainer) {
...
initTTS: () => {
    ...
    const ttsGenderSelect = document.getElementById('ttsGenderSelect');
    if (ttsEngineSelect) ttsEngineSelect.onchange = () => App.speakZh("切換語音引擎測試", 1.0);
    if (ttsGenderSelect) ttsGenderSelect.onchange = () => {
    ...
    if (rateSlider && rateVal) {
        rateSlider.oninput = (e) => { rateVal.textContent = e.target.value; };
    ...
    if (pitchSlider && pitchVal) {
        ...
        pitchSlider.onchange = () => {
            if (App._getTTSEngine() === 'browser') App.speakZh("音調測試", 1.0);
        };
    }
},
...
_getScenarioLabel: (key) => {
    const labels = {
        weather: '氣象與防災', agri: '農業漁業與畜牧',
        traffic: '交通與物流', industry: '生產與製造',
        finance: '科技金融與醫學', life: '客服零售與生活',
        fire: '消防與公共安全'
    };
    return labels[key] || key;
},

// FIX 7: 統一使用 updateProgress，移除重複的 updateProgressBar
updateProgress: () => {
    const stats = SRSManager.getStats();
    ...
    if (progressText) progressText.textContent = `熟練度: ${percent}% (${reviewed}/${total})`;
...
renderHomeScreen: () => {
    ...
    const mkBadges = (id, max) =>
        (zoneTopics[id] || []).slice(0, max)
            ...
            .join('') || '<span style="opacity:0.4; font-size:0.8rem;">暫無指定主題</span>';
    ...
    container.innerHTML = `
        <div class="home-hero">
            ...
            <div class="hero-stats">
                ...
                <div class="stat-item"><span class="stat-num">${allTopics.length > 0 ? Math.round(learned / allTopics.length * 100) : 0}%</span><span class="stat-lbl">完成度</span></div>
            </div>
        </div>
        ...
        <div class="home-actions">
            ...
            <button class="home-action-btn secondary" id="wrongQuizBtn">
                <i class="fas fa-exclamation-triangle"></i> 錯題補救 (${SRSManager.getWrongTopics().length})
            </button>
        </div>
        ...
        <div class="topology-section glass-panel" style="margin-top: 30px; padding: 20px;">
            ...
            <div id="topologyMapContainer" class="mermaid" style="background: rgba(0,0,0,0.2); border-radius: 12px; padding: 15px; overflow-x: auto;">
                ${App.renderTopologyMap()}
            </div>
        </div>

        <div class="achievement-section glass-panel" style="margin-top: 30px; padding: 20px; margin-bottom: 20px;">
            <h3 style="margin-bottom: 20px; color: var(--accent-color);"><i class="fas fa-trophy"></i> 學習成就牆</h3>
            ${AchievementManager.renderWall()}
        </div>
    `;

    // FIX 9: Mermaid 重新渲染（加 SVG 寬度修正）
    setTimeout(() => {
        const mapContainer = document.getElementById('topologyMapContainer');
        if (mapContainer) {
    ...
    if (randomBtn && allTopics.length > 0) {
        randomBtn.onclick = () => App.loadTopic(allTopics[Math.floor(Math.random() * allTopics.length)]);
    }
    ...
    if (wrongBtn) {
        wrongBtn.onclick = () => {
            if (QuizManager.initWrongQuiz()) {
                App._renderWrongQuizUI('⚠️ 錯題補救挑戰', '正在針對您答錯或不熟悉的題目進行專屬強化測驗。');
            } else {
...
renderTopologyMap: () => {
    ...
    (window.lessonData || []).forEach((unit, uIdx) => {
        const uId = `U${uIdx}`;
        // FIX 9: 清除特殊字元避免 Mermaid 解析錯誤
        const unitTitle = unit.title.replace(/['"<>{}]/g, '');
        mermaidCode += `${uId}("${unitTitle}")\n`;
        ...
        unit.sub_units.forEach((sub, sIdx) => {
            ...
            const subTitle = sub.title.replace(/['"<>{}]/g, '').replace(/\n/g, ' ');
            mermaidCode += `${uId} --> ${sId}("${subTitle}<br/>${percent}%")\n`;
...
_pickVoice: (langPrefix, gender, engine) => {
    ...
    const matchesGender = (v) => {
        ...
        if (langPrefix === 'zh') {
            if (!isFemale && (n.includes('szewai') || n.includes('zhiwei') || n.includes('yunyang'))) return true;
